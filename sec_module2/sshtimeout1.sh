...
if test -f "$file"; then
	...
	fExtension=".conf"
	fExtension1=".log"
	backupFile=/etc/ssh/$timeStamp$fExtension
	logName="script"
...
#This creates a backup of the configuration file.
	...
	echo "ClientAliveInterval:`\c`"
	read interval
	echo "ClientAliveCountMax:`\c`"
	read count
...
#This tests if logfile is already created.
	if test -f "$logFile"; then 
		if [ $STATUS -eq 0 ]; then
			...
			echo "$USER $timeStamp Exit_Status: Failed to Execute: `./sshtimeout1.sh`" >> $logFile
